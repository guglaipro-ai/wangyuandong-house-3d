...
def add_fixtures(ns):
 ...
 items=[
  ...
  (2,'廁所B','洗手盆',1.84,7.88,0,180),(2,'廁所B','小便斗',.83,7.88,0,180),(2,'廁所B','坐式便器',2.84,7.88,0,180),
 ...
 for f,room,typ,x,y,up,angle in items:
  ...
  audit.begin(f'fixture_{f}_{room}_{typ}','衛浴設備',f'{f}F {room} {typ}',[6 if f==1 else 7,20],'原模型缺少圖示衛浴設備',f'補建{typ}可見三維外形，位於約({x:.2f},{y:.2f})m','平面符號及配置；產品尺寸／曲面為概念近似')
