...
lines=['# 本次 11 項修正與模型檢查清單','', '2026-09-10（台灣時間）。四套家具風格均已套用；建築原版同步更新房間名稱及衛浴。','', '用途及設備依使用者最新指示；PDF 本身未修改。鄰房位置是避讓基地與道路的概念調整，不是新的測量成果。','', '|項次|位置|已執行修正|','|---|---|---|']
...
page=page.replace('<h2>模型檢查</h2>','<h2>四樓層配置預覽</h2><p>以下為波西米亞版的實際模型截圖；其餘三套採相同用途與家具位置。</p>'+''.join(f'<details><summary>{i} 樓配置</summary><a href="floor-{i}.png"><img loading="lazy" src="floor-{i}.png" alt="{i} 樓配置"></a></details>' for i in range(1,5))+'<h2>模型檢查</h2>')
